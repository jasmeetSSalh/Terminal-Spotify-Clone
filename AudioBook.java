...
public class AudioBook extends AudioContent
{
  ...
  // Print information about the audiobook. First print the basic information of the AudioContent 
	...
	{
		super.printInfo();															//super.printInfo() is the same as AudioContent.printInfo()
		System.out.println("Author: "+this.author + " Narrated by: "+this.narrator + "\n");    //prints the author and narrator
	}
	...
	public void play()
	{
		var chapterToPlay = getChapterTitles().get(currentChapter) + '\n' + getChapters().get(currentChapter);  //gets the chapter title and chapter
		setAudioFile(chapterToPlay);																			//sets the audio file to the chapter title and chapter		
		super.play();																				  			//plays the audio file through the super class
	}
	...
	{
		for (int i = 0; i < chapterTitles.size(); i++) { 									//for loop to print the chapter titles
			System.out.println("Chapter "+(i+1)+ ". "+ chapterTitles.get(i));				//prints the chapter number and chapter title
	...
	{
		if(this.author.equals(((AudioBook)other).author) &&  this.narrator.equals(((AudioBook)other).narrator)&&super.equals(other)) {
			return true;
		} else{
			return false;
		}	}
